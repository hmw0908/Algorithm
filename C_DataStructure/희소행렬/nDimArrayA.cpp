	...
	while (1) {
		...
		while (1) {
			if (ReadIndex(nArrDesc) == false)		// 배열의 색인들을 입력
				break;
			PrintIndex(nArrDesc);
			if (Aij(Arr, nArrDesc, nVal, nErrNdx))	// 색인에 대응하는 배열의 값
...
{	// 정의된 배열에 따라 값을 색인에 맞추어 초기화 한다.
	...
	int *nNdx = nArrDesc + 1;						// 각 차원의 색인들
	int *nBound = nArrDesc + nDim + 1;				// 각 차원의 크기들
...
{	// 배열의 차원과 각 차원의 크기도 입력받아 배열의 차원을 반환한다(에러: -1)
	...
	for (int i = 0; i < nDim; i++) {
		...
		do {
			...
			OK = nBound[i] >= 2 && nBound[i] <= 10;	// 차원의 크기 체크
		} while (OK == false);
	}
	printf("\n\t정의된 배열: int A");
...
{	// 배열 설명자에 있는 색인에 해당하는 원소를 nValue에 저장하고 T/F를 반환한다.
	...
	// nArrDesc은 예제 ArrIndex4에서 모든 매개변수 nDim, nNdx, nBound를 순서대로 넣은 것이다.
	// nArrDesc[0]: 배열의 차원(n)[nDim]
	// nArrDesc[1]  ~  nArrDesc[n]:  배열 각 차원의 색인[nNdx]
	// nArrDesc[n+1] ~ nArrDesc[2n]: 배열 각 차원의 크기[nBound]
	int nDim = nArrDesc[0];							// 배열의 차원
	int *nNdx = nArrDesc + 1;						// 각 차원의 색인들
	int *nBound = nArrDesc + nDim + 1;				// 각 차원의 크기들
	int nSize = 1, nCtr = 0;
	for (int i = nDim - 1; i >= 0; i--) {
		if (nNdx[i] < 0 || nNdx[i] >= nBound[i])	// 범위 초과
			return (nErrNdx = i) != i;
		nCtr += nNdx[i] * nSize;
		nSize *= nBound[i];
	}
